/**
 * Solution to the ride hailing problem using Dinic's algorithm
...
public class DinicRideHailing {
  ...
  static double maxFlow(List<Edge>[] graph, int src, int dest) {
    double flow = 0;
  ...
  static double dinicDfs(List<Edge>[] graph, int[] ptr, int[] dist,
                         int dest, int u, double f) {
    if (u == dest) 
      return f;
    for (; ptr[u] < graph[u].size(); ++ptr[u]) {
      Edge e = graph[u].get(ptr[u]);
      if (dist[e.v] == dist[u] + 1 && e.f < e.cap) {
        double df = dinicDfs(graph, ptr, dist, dest, e.v,
