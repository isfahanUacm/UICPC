...
int main()
{
	...
	for (int i = 0; i < i_name.size(); i++)
	{
		...
		{
			output_i.insert(make_pair(n_email[i], i));
			is_match = true;
		}
	}
	for (auto x : output_i)
	{
		cout << "I " << x.first << " " << n_last[x.second] << " " << n_name[x.second] << endl;
	}
	...
	for (int i = 0; i < o_name.size(); i++)
	{
		...
		{
			input_i.insert(make_pair(no_email[i], i));
			is_match = true;
		}
	}
	for (auto x : input_i)
	{
		cout << "O " << x.first << " " << no_last[x.second] << " " << no_name[x.second] << endl;
	}
