...
int main()
{
	...
	for (int i = 0; i < i_name.size(); i++)
	{
		...
		{
			output_i.insert(make_pair(i_email[i], i));
			is_match = true;
		}
	}
	for (auto x : output_i)
	{
		cout << "I " << n_email[x.second] << " " << n_last[x.second] << " " << n_name[x.second] << endl;
	}
	set<pair<string, int>> input_i;
	for (int i = 0; i < o_name.size(); i++)
	{
		...
		{
			input_i.insert(make_pair(o_email[i], i));
			is_match = true;
		}
	}
	for (auto x : input_i)
	{
		cout << "O " << no_email[x.second] << " " << no_last[x.second] << " " << no_name[x.second] << endl;
	}
