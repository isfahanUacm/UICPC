...
public class tobi_treeset_strtok {
	...
	public static void dijkstra(int from) {
		...
		dist.set(from, 0);
		TreeSet<pii> pq = new TreeSet<pii>();
		...
		while (!pq.isEmpty()) {
			...
			for (Integer next : neighbours) {
				...
				if (dist.get(node) < Integer.MAX_VALUE) {
					pq.remove(new pii(dist.get(node), node));
				}
	...
	public static void main(String args[]) throws IOException {
		...
		line = reader.readLine(); // skip numbe of cases
		while (line != null) {
			StringTokenizer strtok = new StringTokenizer(line);
			numNodes = Integer.parseInt(strtok.nextToken());
			numEdges = Integer.parseInt(strtok.nextToken());
			...
			for (int i = 0; i < numEdges; i++) {
				...
				cost = Integer.parseInt(strtok.nextToken());
			...
			idols.add(0);
			numIdols = Integer.parseInt(reader.readLine());
			...
			air = Integer.parseInt(reader.readLine());
			int best = 0;
			pow2[0] = 1;
