...
public class tobi_strlenstrlenstrlen {
	public static void main(String args[]) {
		...
		while (numCases --> 0) {
			...
			for (int i = 0; i < numSuff; i++) {
				...
				for (int j = 0; j < suff.length(); j++) {
					if (suffChars[suff.length() - j - 1] == '*') {
						break;
					} else if (bufChars[len - j - 1] == '*' || bufChars[len - j - 1] == suffChars[suff.length() - j - 1]) {
						bufChars[len - j - 1] = suffChars[suff.length() - j - 1];
					} else {
						possible = false;
						break;
					}
