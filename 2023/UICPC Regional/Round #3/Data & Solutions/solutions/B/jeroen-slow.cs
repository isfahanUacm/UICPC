...
{
	public static bool[] done = new bool[1<<24];
	public static bool[] mem = new bool[1<<24];
	...
	public static void Main(string[] args)
	{
		...
		while(tests-- > 0)
		{
			...
			while(state < (1<<24)-1)
			{
				...
				{
					if((m & state) != m && !isWinning(m | state))
					{
						move = m;
						break;
					}
	...
	public static bool isWinning(int mask)
	{
		if(done[mask]) return mem[mask];
		done[mask] = true;
		if(mask == (1<<24)-1) return mem[mask] = false;
		foreach(int move in moves)
		{
			if((move & mask) == move) continue;
			if(!isWinning(mask | move)) return mem[mask] = true;
		}
		return mem[mask];
	}
}
