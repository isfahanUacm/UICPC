...
unsigned long long sol (unsigned long long n, unsigned long long maxdigit, int depth) {
	...
	if ((depth > maxdepth) || (maxpossible < n))
		return INFINITY/b - b;	
	for (unsigned long long i = maxdigit; i >= mindigit; i--)	// Optimisation 3: all digits except the last one have size >= sqrt(B)
		if ((n % i) == 0) {
