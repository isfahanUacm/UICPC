...
ll dijkstra(ll c, ll s,ll t){
    ...
    while(!q.empty()){
        ...
        //     if(dist[u][i] > dist[u][fuel] + p[u]*(i-fuel)){
        //         q.erase({dist[u][i],{i,u}});
        //         dist[u][i] = dist[u][fuel] + p[u]*(i-fuel);
        //         q.insert({dist[u][i],{i,u}});
        //     }
        // }
        if(fuel < c){
            if(dist[u][fuel+1] > dist[u][fuel] + p[u]){
                q.erase({dist[u][fuel+1],{fuel+1,u}});
                dist[u][fuel+1] = dist[u][fuel] + p[u];
                q.insert({dist[u][fuel+1],{fuel+1,u}});
            }
        }
        for(auto v:adj[u]){
            if(fuel >= v.second){
