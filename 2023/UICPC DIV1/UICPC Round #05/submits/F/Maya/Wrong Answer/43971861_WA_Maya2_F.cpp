...
bool valid() 
{
    ...
    mark[ib][jb] = 1;
    mark[iy][jy] = 1;
    if (!v(ig, jg, 'G', 1, gg, ig2, jg2)) return false;
    if (!v(ir, jr, 'R', 1, rr, ir2, jr2)) return false;
    if (!v(ib, jb, 'B', 1, bb, ib2, jb2)) return false;
    if (!v(iy, jy, 'Y', 1, yy, iy2, jy2)) return false;
