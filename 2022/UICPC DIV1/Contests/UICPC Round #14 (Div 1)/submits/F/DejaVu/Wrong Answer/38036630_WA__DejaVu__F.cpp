...
void pre_and_then2(string  , string  , string  , int &);
...
int main(void)
{
    ...
    if(find_revers(str1 , str2 , tmp , result))
    {
        ...
        //pre_and_then2(str1 , str2 , tmp , result);
...
void pre_and_then2(string str1 , string str2 , string tmp , int &result)
{
    int MainIndx = str1.find(tmp);
    string tmpPre , tmpLst;

    for(int i = 2 ;; i++)
    {
        if(MainIndx - i < 0)
            break;
        tmpPre = str1.substr(MainIndx - i , i);
        tmpLst = str2.substr(MainIndx + tmp.length() , i);
