...
int main() {
	...
	while (t--) {
		...
		for (ll i = 0;i < rings - 1;i++) {
			...
			bool seen_enter = true, seen_target = false;
			for (ll d = (enter.t2 + 1) % 360;d != enter.t2;d = (d + 1) % 360) {
			...
			seen_enter = true, seen_target = false;
			for (ll d = (enter.t1 - 1 + 360) % 360;d != enter.t1;d = (d - 1 + 360) % 360) {
				if (!seen_enter && contains(enter.t1, enter.t2, d)) {
					seen_enter = true;
					// cout << "1) enter at " << d << endl;
				}
